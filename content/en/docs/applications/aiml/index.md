...
It have turned out that some very large neural language models can generalize so well over natural language that it cal solve some logical and mathematical problems, follow instructions, reason about emotions mental states (sic!), write program code, translate from one language to another, change style of a text, summarize/elaborate and maintain conversation.
